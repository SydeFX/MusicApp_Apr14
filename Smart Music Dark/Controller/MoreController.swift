...
//  Smart Music Dark
//
//  Created by Lyheang IBell on 11/30/18.
...
class MoreController: UIViewController {
    let LOGOUT_INDEX = 4
    let items = [
        (name:"Helps & Support", image:"help"),
        (name:"About Us", image:"about"),
        (name:"Rate App", image:"star"),
        (name:"Version 1.0", image:"version"),
...
extension MoreController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if(indexPath.row == 0) {
            ...
            let configuration = FeedbackConfiguration(toRecipients: ["lyheang.ibell@gmail.com"], usesHTML: true)
            let controller = FeedbackViewController(configuration: configuration)
            controller.title = "Helps & Support"
